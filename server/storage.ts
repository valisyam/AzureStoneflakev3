...
export interface IStorage {
  ...
  generateSupplierNumber(): Promise<string>;
  assignSupplierNumber(userId: string, supplierNumber: string): Promise<void>;
  isSupplierNumberAvailable(supplierNumber: string): Promise<boolean>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getCompanyByCustomerNumber(customerNumber: string): Promise<Company | undefined> {
    const [company] = await db.select().from(companies).where(eq(companies.customerNumber, customerNumber));
    return company;
  }
  ...
  async createCompany(insertCompany: InsertCompany): Promise<Company> {
    if (!insertCompany.customerNumber) {
      insertCompany.customerNumber = await this.generateCustomerNumber();
    }
    ...
    return await db
      ...
      .where(
        or(
          ...
          sql`lower(${companies.customerNumber}) like lower(${'%' + query + '%'})`,
  ...
  async generateCustomerNumber(): Promise<string> {
    // Get all customer numbers to find the highest numeric value
    const allCompanies = await db
      .select({ customerNumber: companies.customerNumber })
      .from(companies)
      .where(sql`${companies.customerNumber} IS NOT NULL AND ${companies.customerNumber} LIKE 'CU%'`);
    ...
    for (const company of allCompanies) {
      const match = company.customerNumber?.match(/CU(\d+)/);
      if (match) {
  ...
  async isCustomerNumberAvailable(customerNumber: string): Promise<boolean> {
    const [existingCompany] = await db
      ...
      .from(companies)
      .where(eq(companies.customerNumber, customerNumber))
      .limit(1);
  ...
  async mergeCompanies(primaryCompanyId: string, companyIdsToMerge: string[], customerNumber: string, companyName: string): Promise<void> {
    // Update the primary company with the new name and customer number
    await db.update(companies)
      .set({ 
        name: companyName, 
        customerNumber 
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    // Generate supplier number for suppliers
    if (insertUser.role === 'supplier' && !insertUser.supplierNumber) {
      insertUser.supplierNumber = await this.generateSupplierNumber();
    }
  ...
  // Generate supplier number (V0001, V0002, etc.)
  async generateSupplierNumber(): Promise<string> {
    // Get the latest supplier number
    const latestSupplier = await db.select().from(users)
      .where(and(eq(users.role, 'supplier'), sql`${users.supplierNumber} IS NOT NULL`))
      .orderBy(desc(users.supplierNumber))
      .limit(1);
    ...
    const lastNumber = latestSupplier[0].supplierNumber?.slice(1); // Remove 'V'
    const nextNumber = lastNumber ? parseInt(lastNumber) + 1 : 1;
  ...
  async createQuote(insertQuote: InsertSalesQuote): Promise<SalesQuote> {
    ...
    const quoteWithNumber = { ...insertQuote, quoteNumber };
    const [quote] = await db.insert(salesQuotes).values(quoteWithNumber).returning();
    return quote;
  }
  ...
  async createInvoice(insertInvoice: InsertSalesInvoice): Promise<SalesInvoice> {
    ...
    const invoiceWithNumber = { ...insertInvoice, invoiceNumber };
    const [invoice] = await db.insert(salesInvoices).values(invoiceWithNumber).returning();
    return invoice;
  }
  ...
  async getAllCustomers(): Promise<(User & { company?: Company })[]> {
    ...
    return usersWithCompanies.map(row => ({
      id: row.id,
      email: row.email,
      ...
      companyNameInput: row.companyNameInput,
      isAdmin: row.isAdmin,
  ...
  async generateSupplierNumber(): Promise<string> {
    // Get all existing supplier numbers and sort them properly
    const existingSuppliers = await db
      .select({ supplierNumber: users.supplierNumber })
      .from(users)
      .where(and(
        eq(users.role, 'supplier'),
        sql`${users.supplierNumber} IS NOT NULL AND ${users.supplierNumber} != ''`
      ));
    ...
    const numbers = existingSuppliers
      ...
      .filter(num => num && num.match(/^V\d{4}$/))
  ...
  async assignSupplierNumber(userId: string, supplierNumber: string): Promise<void> {
    // First check if this vendor number already exists and get the company name
    const existingSupplierWithSameVendorNumber = await db
      .select()
      .from(users)
      .where(and(
        eq(users.role, 'supplier'),
        eq(users.supplierNumber, supplierNumber)
      ))
      .limit(1);

    // If vendor number exists, use the existing company name to maintain consistency
    let updateData: any = { supplierNumber };
    if (existingSupplierWithSameVendorNumber.length > 0) {
      const existingCompanyName = existingSupplierWithSameVendorNumber[0].companyNameInput;
      if (existingCompanyName) {
        updateData.companyNameInput = existingCompanyName;
      }
    }
  ...
  async getExistingCompanyNameForVendorNumber(vendorNumber: string): Promise<string | null> {
    const [existingSupplier] = await db
      .select({ companyNameInput: users.companyNameInput })
      .from(users)
      .where(and(
        eq(users.role, 'supplier'),
        eq(users.supplierNumber, vendorNumber)
      ))
      .limit(1);

    return existingSupplier?.companyNameInput || null;
  }

  async isSupplierNumberAvailable(supplierNumber: string): Promise<boolean> {
    const [existingSupplier] = await db
      .select()
      .from(users)
      .where(eq(users.supplierNumber, supplierNumber));
  ...
  async createSupplierQuote(quote: InsertSupplierQuote): Promise<SupplierQuote> {
    ...
    // Add SQTE number to quote
    const quoteWithSqte = {
      ...quote,
      sqteNumber: rfq.sqteNumber
    };
    
    const [newQuote] = await db.insert(supplierQuotes).values(quoteWithSqte).returning();
  ...
  async getAssignedRfqsBySupplierId(supplierId: string): Promise<(RfqAssignment & { rfq: RFQ & { files: File[] } })[]> {
    const results = await db
      ...
      .where(eq(rfqAssignments.supplierId, supplierId))
  ...
  async updateRfqAssignmentStatus(rfqId: string, supplierId: string, status: 'quoted' | 'expired'): Promise<void> {
    await db.update(rfqAssignments)
      ...
      .where(and(
        ...
        eq(rfqAssignments.supplierId, supplierId)
      ));
  }
  ...
  async getMessageThreads(userId: string): Promise<Array<{ threadId: string; subject: string; category: string; otherUser: User; lastMessage: Message; unreadCount: number }>> {
    ...
    for (const message of allMessages) {
      ...
      if (!threadsMap.has(threadId)) {
        // Get other user info with company information for customer numbers
  ...
  async createMessageAttachment(attachment: InsertMessageAttachment): Promise<MessageAttachment> {
    const [messageAttachment] = await db.insert(messageAttachments).values(attachment).returning();
  ...
  async createPurchaseOrder(purchaseOrder: InsertPurchaseOrder): Promise<PurchaseOrder> {
    ...
    const purchaseOrderWithNumber = { ...purchaseOrder, orderNumber };
    const [newPurchaseOrder] = await db.insert(purchaseOrders).values(purchaseOrderWithNumber).returning();
      ...
      const enrichedOrders = await Promise.all(
        orders.map(async (order) => {
          ...
          return {
            ...
            supplierVendorNumber: supplier?.supplierNumber || 'N/A',
  ...
  async getAllUsersForAdmin(): Promise<Array<{
    ...
    try {
      ...
      return result.rows.map((row: any) => ({
        ...
        supplierNumber: row.supplier_number || undefined,
