...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/auth/me', authenticateToken, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        companyNameInput: fullUser.companyNameInput,
        supplierNumber: fullUser.supplierNumber,
  ...
  app.get('/api/admin/rfqs/supplier-quotes', authenticateToken, requireAdmin, async (req: any, res) => {
    try {
      // Get all RFQs that have been assigned to suppliers (any status that has assignments)
      const allRfqs = await db
        .selectDistinct()
        .from(rfqsTable)
        .innerJoin(rfqAssignments, eq(rfqsTable.id, rfqAssignments.rfqId))
        .orderBy(desc(rfqsTable.createdAt));

      const rfqsWithQuotes = await Promise.all(
        allRfqs.map(async (result) => {
          const rfq = result.rfqs;
  ...
  // Assign vendor number to supplier (admin only)
  app.post('/api/admin/suppliers/:supplierId/assign-vendor-number', authenticateToken, requireAdmin, async (req: any, res) => {
    try {
      ...
      await storage.assignSupplierNumber(supplierId, vendorNumber);
  ...
  // Generate next available vendor number (admin only)
  app.get('/api/admin/suppliers/generate-vendor-number', authenticateToken, requireAdmin, async (req: any, res) => {
    try {
      const vendorNumber = await storage.generateSupplierNumber();
  ...
  app.get('/api/admin/suppliers/check-vendor-number/:vendorNumber', authenticateToken, requireAdmin, async (req: any, res) => {
    try {
      ...
      const existingCompanyName = await storage.getExistingCompanyNameForVendorNumber(vendorNumber);
        ...
        try {
          ...
          if (supplier.email) {
            try {
              ...
              console.error('Failed to send email notification to', supplier.email, emailError);
            }
          }
  ...
  app.post('/api/admin/rfq/:rfqId/assign', authenticateToken, requireAdmin, async (req: any, res) => {
    try {
      ...
      // Send email notifications to all assigned suppliers
      const emailPromises = suppliers.map(async (supplier) => {
        if (supplier && supplier.email) {
          ...
          try {
            ...
            console.error('Failed to send supplier RFQ notification:', error);
          }
        }
        return true;
      });
  ...
  app.get('/api/supplier/purchase-orders', authenticateToken, requireSupplier, async (req: any, res) => {
    try {
      const supplierId = req.user.id;
      const purchaseOrders = await storage.getPurchaseOrdersBySupplierId(supplierId);
  ...
  app.patch('/api/supplier/purchase-orders/:id/status', authenticateToken, requireSupplier, async (req: any, res) => {
    try {
      ...
      // Verify the PO belongs to the current supplier
      const po = await storage.getPurchaseOrderById(poId);
      if (!po || po.supplierId !== req.user.id) {
  ...
  app.get('/api/profile', authenticateToken, async (req: any, res) => {
    try {
      ...
      if (user.companyId) {
        ...
        customerNumber = companyInfo?.customerNumber;
      }
      ...
      res.json({
        ...
        supplierNumber: user.supplierNumber, // Include supplier number for suppliers
        phone: user.phone,
  ...
  app.get("/api/admin/messages/conversations", authenticateToken, requireAdmin, async (req: any, res) => {
    try {
      ...
      const conversations = threads.map(thread => ({
        ...
        supplierNumber: thread.otherUser.supplierNumber || undefined,
  ...
  app.post("/api/admin/messages/send", authenticateToken, requireAdmin, async (req: any, res) => {
    try {
      const { threadId, userId, content, category = 'general' } = req.body;
