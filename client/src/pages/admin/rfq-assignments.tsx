...
export default function RFQAssignments() {
  ...
  const [selectedQuoteForPO, setSelectedQuoteForPO] = useState<any>(null);
  ...
  const createPurchaseOrderMutation = useMutation({
    mutationFn: (data: { quoteId: string; deliveryDate?: string; notes?: string }) =>
      apiRequest(`/api/admin/supplier-quotes/${data.quoteId}/accept`, {
        method: 'POST',
        body: JSON.stringify({ 
    ...
    onSuccess: (data) => {
      toast({
        ...
        description: `Purchase order ${data.purchaseOrder?.orderNumber} has been created successfully.`
      });
      queryClient.invalidateQueries({ queryKey: ['/api/admin/rfq', viewQuotesRfq?.id, 'quotes'] });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6">
            ...
            <div className="space-y-6">
                          ...
                          <div className="flex items-start space-x-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600 mb-2">
                                {supplier.email} • {supplier.country || 'Location not specified'}
                              </p>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button 
                  ...
                  disabled={selectedSuppliers.length === 0 || assignRfqMutation.isPending}
                >
                  <Send className="h-4 w-4 mr-1" />
                  {assignRfqMutation.isPending ? 'Assigning...' : `Assign to ${selectedSuppliers.length} Supplier${selectedSuppliers.length !== 1 ? 's' : ''}`}
                </Button>
              </div>
            </div>
          </DialogContent>
            ...
            <div className="space-y-6">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  ...
                  <div><strong>Process:</strong> {viewQuotesRfq.manufacturingProcess || 'Any'}</div>
                </div>
              </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="space-y-3 flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                              <div className="flex items-center gap-2">
                                ...
                                <span><strong>Price:</strong> ${quote.totalPrice}</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <Clock className="h-4 w-4 text-gray-400" />
                                <span><strong>Lead Time:</strong> {quote.leadTimeWeeks} weeks</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <Package className="h-4 w-4 text-gray-400" />
                                <span><strong>MOQ:</strong> {quote.minimumOrderQuantity}</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <Calendar className="h-4 w-4 text-gray-400" />
                                <span><strong>Submitted:</strong> {format(new Date(quote.createdAt), 'MMM dd, yyyy')}</span>
                              </div>
                            </div>
                          ...
                          {quote.status === 'pending' && (
                            <div className="flex gap-2 ml-4">
                              <Button
                                size="sm"
                                onClick={() => setSelectedQuoteForPO(quote)}
      ...
      {selectedQuoteForPO && (
        <Dialog open={!!selectedQuoteForPO} onOpenChange={() => setSelectedQuoteForPO(null)}>
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div><strong>Supplier:</strong> {selectedQuoteForPO.supplierName}</div>
                  <div><strong>Price:</strong> ${selectedQuoteForPO.totalPrice}</div>
                  <div><strong>Lead Time:</strong> {selectedQuoteForPO.leadTimeWeeks} weeks</div>
                  <div><strong>MOQ:</strong> {selectedQuoteForPO.minimumOrderQuantity}</div>
                </div>
                {selectedQuoteForPO.notes && (
                  <div className="mt-3">
                    <strong>Quote Notes:</strong>
                    <p className="text-gray-600 mt-1">{selectedQuoteForPO.notes}</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <Label htmlFor="poNotes">Purchase Order Notes (Optional)</Label>
                  <Textarea
                    id="poNotes"
